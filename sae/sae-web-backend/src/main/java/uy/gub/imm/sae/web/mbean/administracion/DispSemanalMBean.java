...
public class DispSemanalMBean extends BaseMBean {
	...
	public void beforePhase(PhaseEvent event) {
		...
		if (event.getPhaseId() == PhaseId.RENDER_RESPONSE) {
			sessionMBean.setPantallaTitulo(getI18N().getText("disponibilidades.create.semanal.title"));
		}
	}
	...
	public void consultarSemana(ActionEvent event) {
		...
		if (semanaPatron == null) {
			addErrorMessage(getI18N().getText("disponibilidades.query.week_pattern_required"), MSG_ID);
		}
		else {
	...
	public void generar(ActionEvent event) {
		...
		if (semanaPatron == null) {
			...
			addErrorMessage(getI18N().getText("disponibilidades.query.period_required"), MSG_ID);
		}
			...
			if (haySolapamiento) {
				addErrorMessage(getI18N().getText("disponibilidades.query.period_overflow"), MSG_ID);
			}
			else {
	...
	private void llenarHorariosSemanales(List<List<Object>> horariosSemanales, List<DisponibilidadReserva> disponibilidades) {
		...
		for (DisponibilidadReserva d : disponibilidades) {
			...
			//Uso el indice del orden de dias en la semana segun el Locale para acceder a las columnas de la matriz

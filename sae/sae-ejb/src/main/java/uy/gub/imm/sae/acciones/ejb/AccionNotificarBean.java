...
import uy.gub.imm.sae.business.api.acciones.ejb.EjecutorAccionLocal;
import uy.gub.imm.sae.business.api.acciones.ejb.EjecutorAccionRemote;
...
public class AccionNotificarBean implements
	...
	private final String PROP_MAIL_FROM = "uy.gub.imm.sae.acciones.notificar.mail.from";
	...
	//@Resource(mappedName="java:/sae/mail")  JBOSS7
	...
	private ResultadoAccion notificar(ReservaDTO reserva, RecursoDTO recurso, String mail, Map<String, Object> datos, Boolean confirmacion) 
        ...
        try    {
        	...
        	sendMail(mailSession, mail, subject, content, CONTENT_TYPE_HTML);
        }
	...
	private void sendMail(Session mailSession, String to, String subject, StringBuffer content, String type) throws MessagingException {
		
		Properties saeProperties = SAEProfile.getInstance().getProperties();
		
        MimeMessage m = new MimeMessage(mailSession);
        Address from = new InternetAddress(saeProperties.getProperty(PROP_MAIL_FROM));

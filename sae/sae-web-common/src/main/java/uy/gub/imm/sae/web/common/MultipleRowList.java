...
public class MultipleRowList<T> extends RowList<T> {

	private static final long serialVersionUID = 1L;
	
	private List<Row<T>> selectedRows;
	
	public MultipleRowList() {
		super();
	}
	
	public MultipleRowList(List<T> lista) {
		super(lista);
		for (T elem : lista) {
	...
	public void clear() {
		super.clear();
		setSelectedRows(new ArrayList<Row<T>>());
	}
	
	public List<Row<T>> getSelectedRows() {
		return selectedRows;
	}

	public void setSelectedRows(List<Row<T>> selectedRows) {

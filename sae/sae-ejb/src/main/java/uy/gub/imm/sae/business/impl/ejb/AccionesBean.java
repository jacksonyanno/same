...
import uy.gub.imm.sae.business.api.AccionesLocal;
import uy.gub.imm.sae.business.api.AccionesRemote;
...
public class AccionesBean implements AccionesLocal, AccionesRemote {
	...
	public Accion crearAccion(Accion v)throws UserException, BusinessException {
		...
		if (v.getServicio() == null || v.getServicio().equals("")) {
			throw new UserException("-1", "Debe indicar el nombre del servicio que implementa la acción");
		}
		...
		if (v.getHost() == null || v.getHost().equals("")) {
			throw new UserException("-1", "Debe indicar el nombre del host donde se encuentra la acción");
		}
		...
		//Se controla que no exista otra accion con el mismo nombre
		if (existeAccionPorNombre(v.getNombre()) ) {
				throw new UserException("AE10031","Ya existe una accion con ese nombre "+ v.getNombre());
		}
		//Se controla que la accion no tenga fecha de baja
		if (v.getFechaBaja() != null){
			throw new BusinessException("AE10028","No se puede dar de alta una accion con fecha de baja: "+ v.getNombre());			
		}

		Accion vNueva = new Accion(v);
		em.persist(vNueva);
		
		logger.error("Cant parametros nuevos: "+v.getParametrosAccion().size());
	...
	public void eliminarAccion(Accion v) throws UserException, BusinessException {
		...
		if (accion == null) {
			throw new BusinessException("AE10029","No existe la accion que se quiere eliminar: " + v.getId().toString());
		}
		
		//Se controla que no existan datos asociados vivos para la accion.
		 if  (hayDatosVivos(accion)) {
			throw new UserException("AE10030","La acción se encuentra asociada a algun recurso");
		}
	...
	public Accion modificarAccion(Accion v) throws UserException, BusinessException {
		...
		if (accionActual == null) {
			throw new BusinessException ("AE10032","No existe la accion que se quiere modificar: " + v.getId().toString());
		}
		...
		//Se controla que no exista otra accion viva con el mismo nombre salvo la propia.
		if (existeAccionPorNombreSalvoEsta(v) ) {
			throw new UserException("AE10031","Ya existe una accion con el nombre: "+ v.getNombre());
		}

		//Se controla que la accion no tenga fecha de baja
		if (accionActual.getFechaBaja() != null){
			throw new BusinessException("AE10028","No se puede modificar una accion con fecha de baja: "+ v.getNombre());			
		}
	...
	private void chequeoNoUsoDelParametro(ParametroAccion p) throws UserException {
		...
		if (cant > 0) {
			throw new UserException("-1", "No se puede modificar/eliminar un parámetro si est� siendo utilizado en las acciones de alg�n recurso");
		}
	}
